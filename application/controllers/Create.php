...
class Create extends MY_Controller
{
    ...
    {
        $upload_dir = '/home/valdiearsanur/kantor/ci-book/ch4/uploads/';
        do {
            // Make code
            ...
            // store in $code
            if (in_array($code, $dirs)) {
                // Yes there is
                $img_dir_name = false;
                // Set to false to begin again
            } else {
                // No there isn't
                $img_dir_name = $code;
                // This is a new name
        ...
        if (!mkdir($upload_dir . $img_dir_name)) {
            $page_data = array(
                'fail' => $this->lang->line('encode_upload_mkdir_error'),
                'success' => false
            );
            $this->load->view('common/header');
            $this->load->view('nav/top_nav');
        ...
        if (!$this->upload->do_upload()) {
            ...
            $page_data['result']       = $this->Image_model->save_image(array(
            ...
            if ($page_data['result'] == false) {
                ...
                $page_data = array(
                    'fail' => $this->lang->line('encode_upload_general_error')
                );
                $this->load->view('common/header');
                $this->load->view('nav/top_nav');
                $this->load->view('create/create', $page_data);
